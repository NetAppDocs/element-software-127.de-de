...
Über die API stehen Zugriffstypen zur Verfügung, die in der Element UI nicht verfügbar sind.
...
* link:../api/reference_element_api_app_b_access_control.html["Informationen zu den für Element APIs verfügbaren Zugriffstypen"]
* link:task_system_manage_enable_ldap_authentication.html["Aktivieren Sie die LDAP-Authentifizierung über die Benutzeroberfläche von Element"]
