...
.. Fügen Sie für jeden Standardzeitserver, den Sie hinzufügen möchten, eine neue Zeile hinzu. Die Standardzeitserver müssen dieselben NTP-Server sein, die auf dem Speichercluster verwendet werden, das Sie in einer <<set-up-the-management-node,Später Schritt>> verwenden.
+
[listing]
----
vi /etc/ntp.conf
...
*** *--storage_mvip (-sm) [MVIP-Adresse]*: Die Management Virtual IP-Adresse (MVIP) des Speicherclusters, auf dem die Element Software ausgeführt wird. Der Management-Node wird mit demselben Speichercluster konfiguriert, der während link:task_mnode_recover.html#configure-time-sync["Konfiguration von NTP-Servern"] verwendet wurde.
