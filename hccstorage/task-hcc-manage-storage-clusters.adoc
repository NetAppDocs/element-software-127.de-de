...
[role="lead"]
Sie können Storage-Cluster zur Bestandsaufnahme der Management-Node-Ressourcen hinzufügen, sodass sie mittels NetApp Hybrid Cloud Control (HCC) gemanagt werden können. Der erste Speicher-Cluster, der während des System-Setups hinzugefügt wurdelink:../concepts/concept_intro_clusters.html#authoritative-storage-clusters["Autorisierende Storage-Cluster"], ist der Standard, aber weitere Cluster können über die HCC-Benutzeroberfläche hinzugefügt werden.
