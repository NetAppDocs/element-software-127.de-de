...
TIP: Details zu den zugehörigen Cluster Schwellenwerten Element API finden Sie link:../api/reference_element_api_getclusterfullthreshold.html["„GetClusterFullThreshold“"] in der _Element Software API-Dokumentation_. Informationen zur Kapazität von Block- und Metadaten finden Sie unter link:../storage/concept_monitor_understand_cluster_fullness_levels.html["Allgemeines zu Cluster-Auslastungsebenen"] in der _Element Software-Dokumentation_.
...
Weitere Informationen zur zugehörigen Element API-Methode finden Sie link:../api/reference_element_api_getclusterstats.html["GetClusterStats"]in der _Element Software API-Dokumentation_.
